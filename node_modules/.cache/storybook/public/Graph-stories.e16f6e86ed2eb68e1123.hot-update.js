...
function GraphVisualizer({
  ...
  const recurseGraph = (vis, node, x, y, ctx, pos, connections, canvas) => {
    if (!vis[node]) {
      ...
      let yOffset = -(neighborCount - 1) * 25;
      for (let i = 0; i < neighborCount; i++) {
        ...
        if (!connections[`${node}-${neighbor}`] && !connections[`${neighbor}-${node}`]) {
          ...
          ctx.lineTo(pos[neighbor] ? pos[neighbor].x - nodeWidth / 2 : newX, pos[neighbor] ? pos[neighbor].y : newY);
          ctx.stroke();
...
//# sourceMappingURL=Graph-stories.e16f6e86ed2eb68e1123.hot-update.js.map