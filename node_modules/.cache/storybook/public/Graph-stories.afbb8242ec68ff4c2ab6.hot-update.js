  ...
  const recurseGraph = (vis, node, x, y, ctx, pos, connections, canvas) => {
    if (!vis[node]) {
      ...
      for (let i = 0; i < neighborCount; i++) {
        ...
        const lineIntersectsNode = pos[neighbor] && Math.abs(pos[neighbor].x - newX) <= nodeRadius && Math.abs(pos[neighbor].y - newY) <= nodeRadius;
        if (!connections[`${node}-${neighbor}`] && !connections[`${neighbor}-${node}`] && !lineIntersectsNode) {
          ctx.beginPath();
          ctx.moveTo(x, y);
...
//# sourceMappingURL=Graph-stories.afbb8242ec68ff4c2ab6.hot-update.js.map