...
function GraphVisualizer({
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    ...
    let connections = {};
    let visited = {};
    recurseGraph(visited, startingPoint, 100, 100, ctx, pos, connections, canvas);
  }, [isChange]);
  ...
  const recurseGraph = (vis, node, x, y, ctx, pos, connections, canvas) => {
    if (!vis[node]) {
      ...
      const totalWidth = neighborCount * (nodeWidth + 50); // Total width of all nodes and spaces
      const xOffset = totalWidth > mainWidth ? -totalWidth / 2 + mainWidth / 2 : 0;
      for (let i = 0; i < neighborCount; i++) {
        const neighbor = neighbors[i];
        const newX = x + xOffset + (nodeWidth + 50) * i;
        const newY = y + 100; // Adjust this value according to your needs

        if (!connections[`${node}-${neighbor}`] && !connections[`${neighbor}-${node}`]) {
          ctx.beginPath();
          ctx.moveTo(x, y);
          ctx.lineTo(pos[neighbor] ? pos[neighbor].x : newX, pos[neighbor] ? pos[neighbor].y : newY);
          ctx.stroke();
...
//# sourceMappingURL=Graph-stories.230da595f0e9f8205306.hot-update.js.map