  ...
  const recurseGraph = (vis, node, x, y, ctx, pos, connections, canvas) => {
    if (!vis[node]) {
      ...
      for (let i = 0; i < neighborCount; i++) {
        ...
        const endX = pos[neighbor] ? pos[neighbor].x - width / 2 : Math.min(newX - width / 2, canvas.width - width / 2);
        const endY = pos[neighbor] ? pos[neighbor].y : Math.min(newY, canvas.height - height / 2);
        ...
        recurseGraph(vis, neighbor, endX, endY, ctx, pos, connections, canvas);
        yOffset += 100;
      }
    }
  };
...
//# sourceMappingURL=Graph-stories.235eb96991c084ae8fab.hot-update.js.map